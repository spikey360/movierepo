...
<?php
$sectitlequery="SELECT title FROM disc_sections WHERE secid='$sec'";
$sectitleres=mysql_query($sectitlequery);
$strow=mysql_fetch_row($sectitleres);
echo $strow[0];
?>
</div>
</td></tr>
...
if(empty($sec)){
$qa="SELECT secid, title, description FROM disc_sections";
$res=mysql_query($qa);
if($res){
...
?>
			...
			<td width="500px">
			<a href="section.php?sec=<?php echo $row[0]?>" >
<?php
			echo "<span>$row[1]</span><br><span class='user'>$row[2]</span>"
?></a>
			</td>
			</table>
			</li>
...
$qa=array("select distinct disc_threads.topic, users.username, disc_threads.tid from (select tid, user, pid from disc_posts order by pid desc) as tbl right join disc_threads on tbl.tid=disc_threads.tid left join users on disc_threads.startedBy=users.uid where disc_threads.sticky=1 and section='$sec'  order by tbl.pid desc","select distinct disc_threads.topic, users.username, disc_threads.tid from (select tid, user, pid from disc_posts order by pid desc) as tbl right join disc_threads on tbl.tid=disc_threads.tid left join users on disc_threads.startedBy=users.uid where disc_threads.sticky=0 and section='$sec' order by tbl.pid desc");
$c=0;
foreach($qa as $query){
...
if(txt.length>0)
sendPostRequest("POST","postNewThread.php","p="+escape(txt)+"&s="+"<?php echo $sec;?>");
}
